/* OPT-52275 START */
Insider.__external.cartReminderConfig52275 = function (builderId) {
    'use strict';

    var self = {};

    var variationId = Insider.campaign.userSegment.getActiveVariationByBuilderId(builderId);
    var $cartIcon = Insider.dom('#basketIcon:visible');
    var classes = {
    ...
    self.checkCartReminderOnSite = function () {
        return paidProducts && paidProducts.productList.length > 0 && totalCartAmount &&
            parseFloat(totalCartAmount) > 0 && !Insider.systemRules.call('isOnCartPage') &&
    ...
    self.setCartReminderHtml = function () {
        var reminderHtml =
            ...
            '       <div class="' + classes.totalCartPrice + '">Total de la cesta: ' +
    ...
    self.setCartReminderItemsHtml = function () {
        ...
        Insider.dom(paidProducts.productList).each(function (index, product) {
            ...
            productHtml +=
                ...
                '</p>' +
                '               <p class="' + classes.itemPrice + '">' + String(product.price).replace('.', ',') +
                ' EUR</p>' +
                '           </div>' +
                '       </div>' +
                '   </a>' +
    ...
    self.setCSS = function () {
        Insider.dom('head').append('<style class="' + classes.customStyle + '">' +
            ...
            '.' + classes.footer + '{text-align: center; background-color: #000000 !important; color: white !important;' + /* OPT-52697 */
    ...
    self.setEvents = function () {
        ...
        Insider.eventManager.once('click.go:to:cart:page:' + variationId, '.' + classes.footer, function () {
            window.location.href = 'https://www.laredoute.es/shoppingbasket/basket.aspx';
        });
    };

    self.showCampaign = function () {
        $cartIcon.addClass(classes.defaultGoal);
